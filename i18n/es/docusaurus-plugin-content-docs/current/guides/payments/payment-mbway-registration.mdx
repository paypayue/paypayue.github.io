---
sidebar_position: 5
description: Guardar el método MB WAY para compras futuras
---
...
3. Consulta del pago o [Consulta de los métodos del cliente](../customers/getPaymentMethods.mdx).
...
> **GET** ​/payments/\{id} <button className="btn-api btn-api--float-right btn-api--move-top" onClick={() => ClickShowIframe({idMenuOption: "payments", idOption: "payments/get/payments/{id}"})}><span>Consultar API &nbsp;&nbsp;&nbsp;{'>'}</span></button>
