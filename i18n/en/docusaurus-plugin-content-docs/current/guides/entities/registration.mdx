...
The complete data structure, as well as the options for _economicActivity_ and _availableMethods_ can be consulted <button className="link-api" onClick={ () => ClickShowIframe( "model", "models", "model/entityregistration") } > <span>here</span> </button>.
